...
export function parseOnlineTradeLines(lines: string[]): ParsedReceipt {
  ...
  for (let i = namesFrom; i < lines.length; i++) {
    ...
    // Вторая буква — ЛЮБАЯ кириллическая (не только строчная): имя может
    // начинаться с аббревиатуры заглавными («СВО для процессора DeepCool
    // LE360 PRO») — со старым `[а-яё]` такая строка не признавалась началом,
    // молча выбрасывалась (nameBuf пуст → continue ниже) и следующие имена
    // сдвигались на одну позицию раньше своих строк таблицы.
    const prev = nameBuf[nameBuf.length - 1];
    const looksLikeStart = /^[А-ЯЁ][А-Яа-яЁё]/.test(line);
