# Стандарт экрана: из чего собирается любой список, карточка и форма

Дата: 2026-09-06. Эталон — «Операции» (`apps/web/src/app/(app)/transactions/page.tsx`).
Всё, что ниже, обязательно для каждого экрана; исключения — только те, что
названы явно. Документ короткий нарочно: это чек-лист, а не история решений
(история — в `ui-consistency-2026-09.md` и `ux-audit-handsfree-2026-08-13.md`).

## Список (реестр сущностей)

Сверху вниз, в этом порядке:

1. **`PageHeader`** — `title`, при необходимости `description` (одно предложение:
   что это и зачем), `actions` — главная кнопка «Добавить». Никаких абзацев
   описания в теле экрана.
2. **`KpiRow` + `KpiCard`** — итоги по текущему разрезу, если у экрана есть
   деньги. Значение — `<Money>`, не строка. Плитки стоят **над** полосой
   фильтров (они считаются по разрезу из неё), в обёртке `px-6 py-4`; ряд —
   три-четыре плитки, две выглядят раздутыми.
3. **`FilterBar`** с полями `FilterField`: `SearchField` (ref → `useListHotkeys`,
   «/» ставит курсор), `PeriodSelect` (+ `DateRangeFields`, если период
   произвольный), измерения `Select` (короткие статичные списки) /
   `Combobox` (сущности с поиском), последним — `FilterReset` (одной высоты с
   полями; свои кнопки «Сброс» не заводить).
4. **`DataTable`** — единственная таблица приложения. Обязательны
   `loading`, `error` + `onRetry`, `empty` (`EmptyState` с `action`),
   `mobileCards` (без них на телефоне таблица не рендерится вовсе). Для дат — `groupBy` + `renderGroupHeader`; для денег —
   `footer`; для курсорной пагинации — `hasMore` / `onLoadMore` /
   `loadingMore` (или `LoadMore` под своим списком). Сырой `<table>` запрещён.
5. **Окно** сущности — `Modal`; крупные сущности (заказ, закупка) открываются
   по адресу через `useUrlDialog`, форма создания — по `?new=1`
   (`useCreateFromUrl`).

Состояние фильтров — в адресе через `useUrlFilters(codec)`; период, который
человек выбирает надолго, — ещё и в `localStorage` (`lib/storage`). Клавиши —
`useListHotkeys({ searchRef, onNew })`.

«Нет активной организации» экраны **не рисуют**: это делает `WorkspaceGate`
в каркасе (загрузка / ошибка / пусто). Экран лишь страхуется
`if (!current) return null;`.

## Форма

- Только `Modal` (`ModalContent size=` md — справочник, lg — операция,
  xl/2xl — документ). `Dialog` и `Sheet` для форм не используются.
- `dirty={…}` у `Modal` — обязателен у любой формы с вводом: закрытие с
  несохранённым спрашивает «Закрыть без сохранения?». Кнопки «Отмена» и
  крестик — через `ModalClose`, чтобы идти тем же путём.
- `onConfirm` у `ModalContent` — главное действие для Cmd/Ctrl+Enter.
- Каждое поле — в `FormField` (label, hint, error, aria). Деньги —
  `MoneyInput`; количество — `Input inputMode="decimal"`; флажки — `Checkbox`;
  даты — `Input type="date"` со значениями из `todayInput()` /
  `toLocalDateInput()` и обратно через `fromLocalDateInput()` (UTC+5, никаких
  `toISOString().slice(0, 10)` и `T12:00:00.000Z` руками).
- Удаление — `ConfirmDialog`.

## Деньги, статусы, даты

- Показ суммы — `<Money value tone>`, в том числе внутри фразы («оплачено
  <Money/> из <Money/>») с `tone="plain"`, если цвет уже задан обёрткой;
  `formatRub` — только в строках (тосты, `hint`/`description`, `aria-label`,
  подсказки recharts). Арифметика — `D/add/sub`
  из `@construct/shared`, никогда `Number(...) + Number(...)`.
- Статус в таблице — `StatusDot`, в карточке документа — `StatusStamp`.
  `Badge` в таблицах, списках и формах не используется (решение №15): группа
  или признак — приглушённым текстом рядом с названием, состояние — `StatusDot`. Словарь статуса живёт в одном
  файле (`components/orders/order-shared.tsx`, `lib/labels.ts`) — копировать
  нельзя, импортировать.
- Даты — `lib/dates.ts` (`formatDate`, `formatDateTime`, `formatDayLabel`) и
  `lib/periods.ts`. Свои `toLocaleDateString` не заводить.

## Отчёт

`FilterBar` → `ReportPeriodFields` (период и «Свой диапазон» — в адресе через
`reportCodec` из `lib/report-filters.ts`, измерения отчёта — плоскими
ключами там же) → «Сброс» → экспорт справа (`ExportButtons`) → `KpiRow` с
`Money` → один `FlowChart` (только в ОПиУ и ОДДС: столбцы прихода/расхода по
периодам и линия итога — прибыль или остаток; сравнение с прошлым периодом —
подсказкой в плитках, не серией на графике) → `DataTable` с `footer` итогов и
`mobileCards`. Других графиков в отчётах нет: кольца, водопады, топ-10 и стеки
сняты 10.09 как шум. Строки
с деталями (заказы клиента в дебиторке) раскрываются через `renderExpanded`,
а не вложенной таблицей. `PeriodPicker` не существует.

## Вкладки, переключатели, секции

- Вкладки состояния внутри экрана или окна — `Tabs`/`TabsList`/`TabsTrigger`;
  вкладки-адреса (разделы отчётов) — `NavTabs` с теми же классами
  (`TAB_CLASS`), активная — `aria-current="page"`. Свои `<nav>` с подчёркиванием
  не заводятся.
- Несколько взаимоисключающих режимов («Список / Плитки», назначение строки
  чека) — `SegmentedControl` (группа радиокнопок, стрелки, тихий активный
  сегмент). `ViewToggle` — обёртка над ним в `FilterField`.
- Карточка с заголовком и содержимым вплотную к краям (таблица, строки
  баланса, график) — `SectionCard title aside`; свои шапки с `border-b` не
  собираются.
- Флажок везде — `Checkbox label hint`, сырой `<input type="checkbox">`
  не пишется.
- Кнопка — только `Button`: иконка в строке (удалить вложение, скрыть
  подсказку) — `variant="ghost" size="icon-sm"`, текстовая ссылка-действие —
  `variant="link" size="sm"`. Фильтр, пришедший извне (drill-down, переход с
  плитки) — `Chip label onRemove`. Сырой `<button>` допустим только там, где
  он не выглядит кнопкой (строка списка, имя в дереве).
- Поля в строках документа (позиции заказа, строки чека) — `FormField compact`
  либо общий ряд заголовков колонок над строками; плейсхолдер подписью не
  служит.

## Карточка сущности

`PageHeader` (с ссылкой назад в `description` или крошками) → `KpiRow` →
таблицы связанных сущностей на `DataTable` (строки кликабельны, ведут в окно
или на экран сущности) → действия в `actions`.

## Каркас

Навигация, шапка, таб-бар и меню собираются из тех же примитивов, что и
экраны: `Button`, `Menu`, `CountBadge`, `Tooltip`, `Select`/`Combobox`,
`Modal` (на телефоне — панель снизу). Свои стили для пунктов меню, чипов и
кнопок в каркасе не пишутся.

Три положения боковой панели и одно меню организации (`WorkspaceSwitcher`):

- **панель 240px** — `variant="panel"`: только имя организации, без подписи;
- **рейка 64px** — `variant="rail"`: квадрат с инициалами на оси иконок,
  подписи пунктов — `Tooltip` справа (не `title`); группа «Ещё» сворачивается;
- **телефон** — панели нет: `variant="bar"` живёт слева в шапке (там же
  «Выйти»), разделы — в окне «Ещё» из таб-бара.

Шапка на планшете (md) прячет подписи «По банку», «не разобрано», «Создать»,
«Поиск» до `lg` — остаются число, счётчик и иконки; крошки пути — только с md.

## Ширины

- У `grid` с брейкпоинтами всегда есть `grid-cols-1`: без явной колонки на
  телефоне неявная `auto` растёт по контенту и выталкивает экран вширь.
- Ряд полей документа — `flex flex-wrap … sm:flex-nowrap`, длинные полосы
  (вкладки, степпер) — либо `flex-wrap`, либо `overflow-x-auto`.
- Проверка: на 375px `document.documentElement.scrollWidth === innerWidth`.

## Как проверять

Перед PR по экрану: `pnpm --filter @construct/web typecheck && pnpm --filter
@construct/web lint`, затем стенд: состояния загрузки / ошибки / пусто,
мобильная ширина (375px), клавиши «/», «n», Cmd+Enter, гвард закрытия формы.
